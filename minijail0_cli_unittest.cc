...
// Valid calls to the mount propagation option.
TEST_F(CliTest, valid_mount_propagation) {
  std::vector<std::string> argv = {"-v", "", "", "/bin/sh"};

  // Mode is optional.
  argv[1] = "--mount-propagation";
  ...
  for (const auto& mode : modes) {
    argv[2] = mode + ":" + "test_dir";
    ASSERT_TRUE(parse_args_(argv));
  }
}

// Invalid calls to the remount mode option.
TEST_F(CliTest, invalid_mount_propagation) {
  std::vector<std::string> argv = {"-v", "", "", "/bin/sh"};
  ...
  // Cannot use mount propagation without |-v|
  argv[0] = "--mount-propagation";
  argv[1] = "shared:/etc";
  ASSERT_EXIT(parse_args_(argv), testing::ExitedWithCode(1), "");
}
